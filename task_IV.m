...
subplot(1,4,1), imshow(qr_1), title("QR 1");
subplot(1,4,2), imshow(one), title("QR 1 (29x29) after applying method 1 to find cell size");
subplot(1,4,3), imshow(qr_2), title("QR 2");
subplot(1,4,4), imshow(two), title("QR 2 (37x37) after applying method 1 to find cell size");

%{

% "Method 3" to find the size of a cell in an image
%Read in image
qr_1 = imread("QR_1.jpg");
%medRes1 = medfilt2(qr_1, [3,3]);
qr_2 = imread('QR_2.jpg');

disp("One");
one = qr_code_reader2(qr_1);

disp("Two");
two = qr_code_reader2(qr_2);
sp = figure('Name', 'Thresholded Image');
subplot(1,4,1), imshow(qr_1), title("QR 1");
subplot(1,4,2), imshow(one), title("QR 1 (16x16) after applying method 3 to find cell size");
subplot(1,4,3), imshow(qr_2), title("QR 2");
subplot(1,4,4), imshow(two), title("QR 2 (37x37) after applying method 3 to find cell size");
%}

%Function to generate a binary array to represent QR code
function qr_code = qr_code_reader2(image)
    ...
    cell_size = get_cell_size_in_img2(cropped_img);
    disp("Cell size: " + cell_size);
    dbl_cell_size = double(cell_size);
    dbl_rows = double(rows);
    dbl_cols = double(cols);
    cells_in_qr_x = floor(dbl_rows/dbl_cell_size);
    cells_in_qr_y = floor(dbl_cols/dbl_cell_size);
    disp(cells_in_qr_x+" "+cells_in_qr_y);
    binarized_img = imbinarize(cropped_img);
    resized_binary_img = imresize(binarized_img, [cells_in_qr_x, cells_in_qr_y]);
...
function cell_size = get_cell_size_in_img2(img)
